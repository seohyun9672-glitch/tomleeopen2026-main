...
import { ROUND_F, ROUND_PRE, ROUND_R16, ROUND_QF, ROUND_SF } from "@/lib/round";

import { categoryDisplayLabelFromDbRow } from "@/lib/categories";
...
// ─── Standings helpers (from standings.ts) ────────────────────────────────────
...
export function teamRowsFromCategoryMatches(matches: MatchWithTeamNames[]): TeamRow[] {
  ...
  for (const m of matches) {
    const sides: Array<{ id: string | null; seed: string | null; display: string | null }> = [
...
function standingsTeamDisplayName(t: TeamRow): string {
...
function computeStandingsAll(prelimMatches: StandingsMatchRow[], teams: TeamRow[]): StandingsRow[] {
  return computeStandingsFromMatches(prelimMatches, teams);
}

function computeStandingsFromMatches(matches: StandingsMatchRow[], teams: TeamRow[]): StandingsRow[] {
  const stats = new Map<string, { W: number; L: number; setsWon: number; setsLost: number; gamesWon: number; gamesLost: number }>();
  ...
  for (const m of matches) {
    ...
    if (set1) {
      s1.setsWon += set1.t1 > set1.t2 ? 1 : 0; s1.setsLost += set1.t1 < set1.t2 ? 1 : 0;
      s2.setsWon += set1.t2 > set1.t1 ? 1 : 0; s2.setsLost += set1.t2 < set1.t1 ? 1 : 0;
      s1.gamesWon += set1.t1; s1.gamesLost += set1.t2;
      s2.gamesWon += set1.t2; s2.gamesLost += set1.t1;
    }
    if (set2) {
      s1.setsWon += set2.t1 > set2.t2 ? 1 : 0; s1.setsLost += set2.t1 < set2.t2 ? 1 : 0;
      s2.setsWon += set2.t2 > set2.t1 ? 1 : 0; s2.setsLost += set2.t2 < set2.t1 ? 1 : 0;
      s1.gamesWon += set2.t1; s1.gamesLost += set2.t2;
      s2.gamesWon += set2.t2; s2.gamesLost += set2.t1;
    }
    if (m.winner === 1) { s1.W++; s2.L++; }
    else if (m.winner === 2) { s2.W++; s1.L++; }
  }
  return teams.map((t) => {
    const st = stats.get(t.id)!;
    return { teamId: t.id, displayName: standingsTeamDisplayName(t), W: st.W, L: st.L, SD: st.setsWon - st.setsLost, GD: st.gamesWon - st.gamesLost };
  }).sort((a, b) => {
...
export function buildTeamRankMapFromPrelims(teams: TeamRow[], prelimMatches: StandingsMatchRow[]): Map<string, number> {
